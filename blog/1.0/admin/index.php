...
if(isset($_SESSION['aid']) && isset($_SESSION['aname']) && isset($_SESSION['apass'])){
	$aid = (int)$_SESSION['aid'];
	$aname = $_SESSION['aname'];
	$apass = $_SESSION['apass'];
	$info = $mysql->get("select * from `admins` where `aid`={$aid} and `aname`='{$aname}' and `apass`='{$apass}'");
	if($aid == $info[0]['aid'] && $aname == $info[0]['aname'] && $apass == $info[0]['apass'] ){
		$admins = $mysql->get("select * from `admins`");
	}else{
	...
	$start = ($page-1)*$limit;
	$admins = $mysql->get("select * from `admins` limit {$start},{$limit}");
}else{
	header("location: login.php");
	exit("重新登录");
}
