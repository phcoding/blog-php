...
if(($result = $input->sessions(['aid','aname']))  && $sname = $input->get('sname')){
    $aid = (int)$result['aid'];
    $aname = $result["aname"];
    $pagetitle = "修改配置";
    if($action = $input->get('action')){
        switch ($action) {
            case 'update':
                if($form = $input->posts(['conf_name','conf_value','conf_inst'])){
                    $conf_name = (string)$form['conf_name'];
                    $conf_value = $form['conf_value'];
                    $conf_inst = (string)$form['conf_inst'];
        ...
        $result = $mysql->get("setting_inst","sname",$sname);
